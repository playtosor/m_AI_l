...
- If a message contains `ANNULS:YYYYMMDD-HHMM`, ignore the referenced message

### 3. Archive silently
Once all messages are understood and doubts resolved:

- Append the full content of `./mailbox_[YOUR_HANDLE].md` to `./mailbox_[YOUR_HANDLE]_archive.md`
- Empty `./mailbox_[YOUR_HANDLE].md` (leave it blank, do not delete it)
- Do not mention the archiving in the conversation unless an error occurs
- **If the append succeeds but the empty operation fails:** do not re-append — the messages are already in context and already archived. Retry the empty operation only.
